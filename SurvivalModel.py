#Problem 1: Five-Year Survival: Modify the survival model so that you can get the
...
class Cohort:
    ...
    def get_5year_survival(self):
        """:return:the percentage of patients survived beyond 5 years """
        survival_5year = 0
        size = len(self._survivalTimes)
        for i in range(0, size):
            if self._survivalTimes[i] > 5:
                survival_5year += 1 / size
            #print(i, self._survivalTimes[i], survival_5year)
        return survival_5year
...
class MultiCohort:
    ...
    def __init__(self, ids, pop_sizes, mortality_probs):
        ...
        self._survival5YearPct = [] # list of five year survival probability
    ...
    def simulate(self, n_time_steps):
        ...
        for i in range(len(self._ids)):
            # create a cohort
            #print(self._ids[i], self._popSizes[i], self._mortalityProbs[i])
            ...
            # store all patient 5 year surival probability from this cohort
            self._survival5YearPct.append(cohort.get_5year_survival())
    ...
    def get_cohort_5yearSurvivalPct(self,cohort_index):
        """ returns the mean 5 year survival percentage
        :param cohort_index: integer over [0, 1, ...] corresponding to the 1st, 2ndm ... simulated cohort
        """
        return self._survival5YearPct[cohort_index]
